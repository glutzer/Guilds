...
float borderShading() {
  bool up = (sideMask & (1 << 0)) != 0;
  bool right = (sideMask & (1 << 1)) != 0;
  bool down = (sideMask & (1 << 2)) != 0;
  bool left = (sideMask & (1 << 3)) != 0;

  float shading = 0;

  if (uv.y > 0.7 && up) {
    shading = max(shading, smoothstep(0.7, 1, uv.y));
  }

  if (uv.y < 0.3 && down) {
    shading = max(shading, smoothstep(0.3, 0, uv.y));
  }

  if (uv.x > 0.7 && right) {
    shading = max(shading, smoothstep(0.7, 1, uv.x));
  }

  if (uv.x < 0.3 && left) {
    shading = max(shading, smoothstep(0.3, 0, uv.x));
  }
